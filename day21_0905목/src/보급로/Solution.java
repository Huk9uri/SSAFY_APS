...
/*
 * 1. 큐를 생성하고 2. 처음 시작할 요소들을 담은 배열 queue에 추가하고 3. 해당 큐에서 꺼내서 해당 좌표를 갔을 때, 처리해야할
 * 로직을 실행하고 4. 로직 실행 후 , 다음 좌표도 범위 안에 있다면 (처리해야할 범위 ,map의 범위 안에 있다면) 5. 그 다음 좌표를
 * 가진 배열을 2번 처럼 추가 하고 2~5까지 반복한다. 6. queue가 비었을 때 // 결국 더 탐색할 게 없다. .--> 탐색 끝 7.
 * 최소치를 갱신했으니까 탐색이 끝난 좌표의 값에는 최소치가 있다.
 */